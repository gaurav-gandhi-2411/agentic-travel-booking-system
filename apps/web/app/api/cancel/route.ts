...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    const body = (await req.json()) as { booking_ref?: unknown };
    booking_ref = typeof body.booking_ref === 'string' ? body.booking_ref.trim() : '';
  } catch {
    return errorSSE('Invalid request body');
  }

  if (!booking_ref) {
    return errorSSE('booking_ref is required');
  }
  ...
  try {
    const upstream = await fetch(`${apiBase}/cancel`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(apiKey ? { 'X-API-Key': apiKey } : {}),
      },
      body: JSON.stringify({ booking_ref }),
