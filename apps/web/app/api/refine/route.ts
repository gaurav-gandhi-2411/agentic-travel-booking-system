...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    const body = (await req.json()) as { request_id?: unknown; refinement?: unknown };
    request_id = typeof body.request_id === 'string' ? body.request_id.trim() : '';
    refinement = typeof body.refinement === 'string' ? body.refinement.trim() : '';
  } catch {
    return errorSSE('Invalid request body');
  }

  if (!request_id || !refinement) {
    return errorSSE('request_id and refinement are required');
  }
  ...
  try {
    const upstream = await fetch(`${apiBase}/refine`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ request_id, refinement }),
