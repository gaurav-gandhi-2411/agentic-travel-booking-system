...
| [0028](0028-supabase-pool-and-sentry-noise-fix.md) | Supabase Transaction Pooler + Sentry Fallback-Noise Filter | Post-fallback hardening | Transaction pooler (port 6543) for runtime DB, explicit pool sizing (200-client math), retry-then-503 on pool exhaustion, schema-qualified ORM models; Sentry `before_send` tag-filter drops recovered fallback-hop noise without blinding other profiles |
